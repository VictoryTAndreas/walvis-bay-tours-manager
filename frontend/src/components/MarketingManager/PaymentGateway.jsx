...
const PaymentGateway = ({ promotions, currentUser }) => {
    ...
    useEffect(() => {
        if (selectedPackageId) {
            const selectedPromotion = promotions.find(p => String(p.id) === String(selectedPackageId));
            if (selectedPromotion) {
                const price = selectedPromotion.price ? parseFloat(selectedPromotion.price.replace(/[^\d.]/g, '')) : 0;
                setFormData(prev => ({
                    ...prev,
                    packageId: String(selectedPromotion.id), // Ensure it's a string
      ...
      <div className="tw-grid md:tw-grid-cols-2 tw-gap-0">
            ...
            <div className="tw-space-y-4">
              <div className="tw-bg-white/10 tw-rounded-lg tw-p-4">
                ...
                <div className="tw-text-sm tw-opacity-90">
                  ...
                  <p className="tw-mt-2 tw-text-2xl tw-font-bold">N$ {formData.amount.toLocaleString()}</p>
                </div>
              </div>
              ...
              <div className="tw-border-t tw-border-white/20 tw-pt-4">
                <div className="tw-flex tw-justify-between tw-items-center tw-text-lg tw-font-semibold">
                  ...
                  <span>N$ {formData.amount.toLocaleString()}</span>
                </div>
              </div>
            </div>
          ) : (
        ...
        <div className="tw-p-8">
          <form onSubmit={handleSubmit} className="tw-space-y-6">
            ...
            {/* Package Selection Section - FIXED */}
            <div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    // FIXED: Added String() conversion for consistent type comparison
                    const selected = promotions.find(p => String(p.id) === String(e.target.value));
                    if (selected) {
                      const price = selected.price ? parseFloat(selected.price.replace(/[^\d.]/g, '')) : 0;
                      setFormData(prev => ({
                        ...prev,
                        packageId: String(selected.id), // Ensure it's a string
                ...
                >
                  ...
                  {promotions.filter(p => p.isActive).map(promotion => (
                    <option key={promotion.id} value={String(promotion.id)}> {/* Ensure value is string */}
                      {promotion.title} - N$ {promotion.price || '0'}
                    </option>
                  ))}
                </select>
            ...
            <div className="tw-pt-4">
              ...
              >
                {isProcessing ? (
                  ...
                  `Pay N$ ${formData.amount.toLocaleString()}`
                )}
              </button>
            </div>
